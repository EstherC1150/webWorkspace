module.exports = {
  //사원 전체 조회
  empList: `SELECT e.emp_no, e.first_name, e.last_name, e.gender, e.hire_date, s.salary, d.dept_no, d.dept_name
              FROM employees e JOIN dept_emp h
                               ON(e.emp_no = h.emp_no)
                               JOIN departments d
                               ON(h.dept_no = d.dept_no)
                               JOIN salaries s
                               ON(e.emp_no = s.emp_no)
              WHERE h.to_date = CAST('9999-01-01' AS DATE)
              AND s.to_date = CAST('9999-01-01' AS DATE)
              ORDER BY e.emp_no DESC
              LIMIT 0, 10`,
  //부서조회
  dept: `SELECT dept_no, dept_name FROM departments ORDER BY dept_no`,
  //사원등록
  //SET 뒤에 ?는 무조건 객체가 넘어 간다고 생각하면 됨
  empInsert: `INSERT INTO employees SET ?`,
  //부서에 등록
  //SET절이라고 무조건 객체가 넘어가는건 아니라 생각하기
  deptEmpInsert: `INSERT INTO dept_emp SET emp_no = ?, dept_no = ?, from_date = ?, to_date = CAST('9999-01-01' AS DATE)`,
  //급여에 등록
  salariesInsert: `INSERT INTO salaries SET ?`,
  //사원수정
  empUpdate: `UPDATE employees
              SET ? 
              WHERE emp_no = ?`,
  //사원삭제 : update로
  empDelete: `UPDATE dept_emp SET to_date = ? WHERE emp_no = ?`,
  //사원단건조회
  empInfo: `SELECT e.emp_no, e.first_name, e.last_name, e.gender, e.hire_date, s.salary, d.dept_no, d.dept_name
            FROM employees e JOIN dept_emp h
                             ON(e.emp_no = h.emp_no)
                             JOIN departments d
                             ON(h.dept_no = d.dept_no)
                             JOIN salaries s
                             ON(e.emp_no = s.emp_no)
            WHERE h.to_date = CAST('9999-01-01' AS DATE)
            AND s.to_date = CAST('9999-01-01' AS DATE)
            AND e.emp_no = ?`,
};
